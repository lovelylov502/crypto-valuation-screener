...
- `신규 감지`: DefiLlama에 등록된 지 30일 이내이거나 최근 7일 수수료 성장률이 50% 이상이면서 30일 수수료·매출·holder revenue 중 가장 큰 값이 $10K 이상인 자산이다.
- `관심종목`: 사용자가 표에서 별표로 고른 자산이며 브라우저에만 저장한다.

두 신호는 후보를 찾고 다시 보는 데만 쓴다. P/HR 정의, 밸류 점수, 가치포획 점수에는 반영하지 않는다. 성장 신호가 holder revenue의 의미를 바꾸거나 포획 점수를 높이지 않는다.
