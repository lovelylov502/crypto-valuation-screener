...
- DefiLlama에 등록된 지 30일 이내이거나 7일 수수료 성장률이 50% 이상이면서 30일 현금흐름이 $10K 이상인 자산을 `신규 감지`로 표시.
- `신규만` 체크 필터를 추가하고 신규 판정·관심종목 직렬화 로직을 순수 함수 테스트로 검증.
- 결측값과 유한하지 않은 숫자가 신규 판정을 통과하지 않도록 경계를 고정했다.
- 신규 감지와 관심종목을 밸류 점수·P/HR·가치포획 계산에서 분리했다.
