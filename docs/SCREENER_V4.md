...
The initial workflow push was rejected because the OAuth credential lacked `workflow` scope. The user authorized that scope on 2026-09-09, completed account reauthentication and enabled the daily workflow. Its first hosted manual run passed collection, artifact upload and website warming. Exact activation and hosted-run evidence are recorded in HANDOFF.md. Website request-based refresh and manual refresh work independently of the scheduled archive.
