...
export interface WorkspacePreferences {
  layoutVersion: 9;
  revenueSort: "amount" | "percent" | "delta";
  growingOnly: boolean; holderOnly: boolean;
...
export const defaultPreferences = (): WorkspacePreferences => ({ layoutVersion: 9, revenueSort: "amount", growingOnly: false, holderOnly: false, columns: [...DEFAULT_VISIBLE_COLUMNS], capital: "mcap", view: "all", search: "", category: "", panel: "columns", sidebarOpen: false, available: "all", coverageWindow: "any", minMcap: 0, maxMcap: 0, maxPr: 0, maxPhr: 0, rangeWindow: 30, hideDilution: false, verifiedOnly: false, sortKey: "pr", sortDir: "asc" });
...
export function workspaceMigrationNotice(raw: string | null): string | null {
  try {
    ...
    if (p && p.layoutVersion !== 9) return "기간별 수익 비교와 행 아래 상세로 바뀌었습니다. 관심종목·직접 설정한 열·필터는 유지됩니다. 기존 기본 열은 전 기간 보기로 바뀝니다.";
...
export function parseWorkspace(raw: string | null): WorkspacePreferences {
  ...
  try {
    ...
    const parsed: WorkspacePreferences = { ...defaults, columns: columns.length ? columns : defaults.columns,
      ...
      growingOnly: p.growingOnly === true || p.view === "growth", holderOnly: p.holderOnly === true || p.view === "holder",
      panel: p.panel === "filters" ? "filters" : "columns", sidebarOpen: false,
      capital: p.capital === "fdv" ? "fdv" : "mcap",
      view: ["all", "favorites", "issues"].includes(p.view) ? p.view : "all",
      ...
      sortKey: oldDefault ? "pr" : keys.has(p.sortKey) || p.sortKey === "name" ? p.sortKey : defaults.sortKey, sortDir: oldDefault ? "asc" : ["asc", "desc"].includes(p.sortDir) ? p.sortDir : defaults.sortDir };
