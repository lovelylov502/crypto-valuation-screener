...
- The CoinMarketCap dark table informed the navy/blue palette, compact toolbar and numeric hierarchy. The full-width table has no permanent sidebar or sort dropdown. Click a column heading to sort; missing values stay last and hiding the sorted column falls back to a visible sort. Details open on the right without replacing the table.
- Display settings use one ordered list with drag handles and keyboard arrows, followed by unselected metrics to add. Restore default columns is fixed at the top and resets only the columns/order (plus any now-hidden sort). Defaults are price, 24-hour change, market cap, 30-day P/R and 30-day P/HR. P/S remains supplementary evidence in coin details and is absent from the main table/filter; legacy sales thresholds are cleared rather than relabeled as revenue. Other saved preferences and watchlists survive migration.
- Whole-universe/watchlist and MCap/FDV remain visible buttons. P/R and P/HR range filters share an explicit 7/30/90/365-day period; source-availability filtering has its own period. The table coverage count uses the currently visible metric periods and filtered rows, deduplicates overlapping coverage, and never includes the separate market-cap reference in FDV counts. An expandable whole-universe overview separates computable, source-present withheld and missing rows without changing filters.
- Column order, numerator choice, watchlist and filters persist in the same browser, including existing V7 preferences; dialogs start closed. New period columns stay beside their metric; the four-period action groups 1 year / 90 / 30 / 7 days without resetting other columns. See [design-qa.md](./design-qa.md) for reference comparison and interaction checks.
