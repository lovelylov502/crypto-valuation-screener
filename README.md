...
가격이 아니라 **펀더멘털**(수수료·매출·**holder revenue**·예치자산)과 **토큰 가치포획**으로 코인의 저평가/고평가를 가늠하는 웹 스크리너.
각 밸류에이션 멀티플을 **같은 섹터 안에서 백분위로 정규화**한 뒤 가중 평균해 **0~100 종합 점수**(높을수록 저평가)로 보여주고, 별도 **가치포획 탭**에서 “이 매출이 토큰 홀더에게 실제로 꽂히는가”를 확인한다.
