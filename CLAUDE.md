...
6. **검토 상태를 늘리지 않는다.** 사용자의 선택은 별표 관심종목 하나로 저장하고, 신규 감지는 체크 필터로만 제공한다.

## 신규 감지·관심종목

- `신규 감지`: DefiLlama에 등록된 지 30일 이내이거나 최근 7일 수수료 성장률이 50% 이상이면서 30일 현금흐름이 $10K 이상인 자산.
- `신규만`: 위 조건을 만족한 현재 스크리너 자산만 표시한다.
- 별표 관심종목은 브라우저 `localStorage`에 저장하며 서버·계정 동기화는 하지 않는다.
- 신규 감지·관심종목 로직은 P/HR·밸류 점수·가치포획 점수에 반영하지 않는다.

## 데이터 소스 (전부 무료, API 키 불필요)

| 소스 | 용도 |
|---|---|
| DefiLlama `/protocols` | name·symbol·category·gecko_id·mcap·tvl·listedAt |
