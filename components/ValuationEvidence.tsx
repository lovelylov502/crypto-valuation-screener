...
import { salesMultiple, salesReason, holderMultiple, holderAmount, holderReason, protocolMultiple, protocolReason, type CapitalBasis } from "@/lib/valuationMetrics";
...
export function ValuationEvidence({ coin: c, capital }: { coin: CoinScored; capital: CapitalBasis }) {
  ...
  return <div className="valuation-evidence">
    ...
    <section className="detail-section">
      <div className="section-title"><h3>P/S · 사업 매출</h3><span>{salesReason(c,capital)}</span></div>
      {s ? <>
    ...
    <section className="detail-section"><div className="section-title"><h3>P/HR · 홀더 환원</h3><span>현재 {capName} {fmtUsd(c[capital])}</span></div>
      ...
      <p className="detail-note muted">{c.holderValue.components.filter(p=>p.eligible).map(p=>p.name).join(", ") || "적격 환원 구성요소 없음"} · 아래에서 포함·제외 방식과 수령 조건을 확인하세요.</p>
