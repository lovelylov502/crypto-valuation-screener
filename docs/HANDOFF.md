...
302 source components have exact reviewed six-field methodology records, including explicit unresolved classifications. The UI/API distinguish protocol revenue, service sales, holder return and estimated trading PnL. New/changed/missing/mixed definitions cannot silently become sales or business-growth signals. Scope review gates holder-share bonuses; raw amounts and source links remain available. Definition and historical-window fingerprints prevent incompatible baseline and detail-history comparisons. Negative amounts and missing parent constituents are preserved; incomplete one-year amounts do not become TTM. Token-identity eligibility now consistently gates cashflow multiples and capture promotion. Arbitrary fee-to-cashflow conversion and mismatched-universe rank comparisons are removed.

Validation: 164 application tests, six deployment-contract tests and TypeScript pass; the production build passed and the guarded deployment repeats all gates against the final committed tree. The 2026-09-19T15:06:39.451Z replay has 724 screened rows, 142 available typed 30-day multiples, 3,974,160 uncompressed API bytes and 755,736 compressed-cache base64 bytes. DefiLlama and CMC succeeded; CoinGecko page 2 failed, so optional market enrichment was partial. Snapshot inputs, source observations and rule hashes are outside Git under the task's work directory.

VVV replay: 797,709 USD over completed 30 days, typed as holder return; no business growth, no P/F, no inferred holder-share ratio; capture score 43 and holder-only improvement contribution 6.3. A reviewed Overtime full-distribution fixture still yields 100%; equality alone does not. Live-data local browser QA confirmed the VVV label/exclusion, exact provider definitions, unavailable partial-year multiple, share withholding, kind/reference interaction, unknown-scope quarantine and incompatible-history notice. No browser warning/error logs were observed. The final source-accounting labels additionally distinguish Revenue and Fees when their economic labels are identical.
