...
`lib/protocolResearch.ts` contains nine Korean project introductions and four detailed policy records (Pendle, Hyperliquid, Liquity V1 and GMX), each with a source, scope and review date. Uncovered projects use the provider's original description where present. These records are a bounded initial researched set, not complete coverage or automatic translations of the whole universe.
