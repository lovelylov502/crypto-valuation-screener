...
- 신규만: DefiLlama에 등록된 지 30일 이내이거나 7일 수수료 성장률이 50% 이상이면서 30일 현금흐름이 $10K 이상인 자산만 표시.
- 관심만: 코인 열의 별표로 선택한 자산만 표시하며 선택값은 브라우저에 저장.
- 섹터: 접힌 다중 선택 목록.
- 전체 초기화: 검색·범위·섹터·체크·정렬을 기본값으로 복구.
- 신규 감지와 관심종목은 클라이언트 필터에만 쓰고 밸류 점수·가치포획 점수에는 반영하지 않는다.
...
- [ ] 신규 감지 배지·신규만 필터와 별표·관심만 필터가 동작한다.
