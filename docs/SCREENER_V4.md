...
The planned `.github/workflows/daily-snapshot.yml` runs at 00:17 and 12:17 UTC (09:17 and 21:17 KST) after the workflow reaches the default branch and Actions is enabled. It uses a standard Ubuntu runner, read-only repository permission and no vendor/API secrets. Artifacts are retained for 60 days. It then warms the canonical website's page and API caches and verifies a fresh matching-version API response. A local successful run does not prove the hosted schedule has run.

Publication status on 2026-09-09: GitHub rejected the workflow push because the active OAuth credential lacks `workflow` scope. The complete workflow is preserved in local branch `codex/screener-signal-ux-overhaul`, commit `ad6afc1722e1d768ad88755f7e9efbcfcfe75b89`. It is intentionally absent from `main` until an authorized credential can publish it. Website request-based refresh and manual refresh work independently; twice-daily unattended capture is not active.

The repository was verified public on 2026-09-08. Standard public-repository Actions minutes are free; artifacts use GitHub's shared storage allowance. The sampled archive was approximately 1.25MB per run, about 150MB at two daily runs for 60 days. Account-wide storage use still determines allowance headroom. Scheduled jobs can be delayed, and public schedules can be disabled after repository inactivity. No exact-time delivery is promised. [GitHub billing](https://docs.github.com/en/billing/concepts/product-billing/github-actions), [schedule behavior](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).
