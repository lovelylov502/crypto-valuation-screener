...
export function deriveOpportunities(
  ...
  if (coin.holderValue.components.some(p => p.eligible) && !holderHistoryMatches(coin)) dataIssues.push("일별 환원 이력 없음·정의 불일치 · P/HR 보류");
  if (coin.holderHistory && coin.holderHistory.periods[30].total === null) dataIssues.push(`30일 환원 이력 ${coin.holderHistory.periods[30].reportedDays}/30일`);
  if (coin.sales && coin.sales.status !== "current") dataIssues.push("외부 매출 근거 재검토 필요 · P/S 보류");
  ...
  const risks: string[] = [];
  if (!eligibleCapital(coin)) dataIssues.push(coin.capitalExclusionReason ?? coin.identityReason);
  if (!gates.marketData)
  ...
  if ((datedHolder.eligibleCurrent30d ?? 0) > 0)
