...
export const defaultPreferences = (): WorkspacePreferences => ({ columns: [...DEFAULT_VISIBLE_COLUMNS], capital: "mcap", view: "all", search: "", category: "", panel: "columns", sidebarOpen: false, available: "all", coverageWindow: "any", minMcap: 0, maxMcap: 0, maxPs: 0, maxPhr: 0, hideDilution: false, verifiedOnly: false, sortKey: "mcap", sortDir: "desc" });
export function parseWorkspace(raw: string | null): WorkspacePreferences {
  ...
  try {
    ...
    return { ...defaults, columns: columns.length ? columns : defaults.columns,
      ...
      available: Object.hasOwn(AVAILABILITY_LABELS,p.available) ? p.available : "all",
