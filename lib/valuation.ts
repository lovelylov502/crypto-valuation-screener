...
// 멀티플(낮을수록 쌈) → 섹터 내 "숌 정도" 백분위(0~100, 높을수록 쌈)
// 값이 가장 작으면 100에 가깝다
...
type SortedPools = {
  byCategory: Map<string, { pf: number[]; ps: number[]; mcapTvl: number[] }>;
  global: { pf: number[]; ps: number[]; mcapTvl: number[] };
};

function buildPools(
  rows: { coin: CoinRaw; m: CoinScored["multiples"]; lowActivity: boolean; insufficientScale: boolean }[],
): SortedPools {
  const empty = () => ({ pf: [] as number[], ps: [] as number[], mcapTvl: [] as number[] });
  ...
  for (const { coin, m, lowActivity, insufficientScale } of rows) {
    ...
    // P/F·P/S는 활동성 좀비를 분포에서 제외 (왜곡 방지). Mcap/TVL은 활동성 무관.
    if (!lowActivity) {
  ...
  global.pf.sort((a, z) => a - z);
  global.ps.sort((a, z) => a - z);
  global.mcapTvl.sort((a, z) => a - z);
  ...
  return staged.map(({ coin, m, lowActivity, insufficientScale }): CoinScored => {
    ...
    if (insufficientScale) {
      return {
        ...coin,
        multiples: m,
        sectorPercentiles: { pf: null, ps: null, mcapTvl: null },
        growthScore: null,
        valueScore: null,
        label: "판단보류",
        confidence: 0,
    ...
    // 밸류 멀티플(P/F·P/S·Mcap/TVL)이 하나도 없으면 성장성/희석만으론 점수 의미 없음 → 판단보류
    ...
    // 신뢰도: 밸류 멀티플 가용성 + 활동성
    let confidence = valueMultipleCount / 3;
    ...
    return {
      ...coin,
      multiples: m,
      sectorPercentiles: { pf: pctPf, ps: pctPs, mcapTvl: pctMcapTvl },
      growthScore,
