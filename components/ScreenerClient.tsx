...
export function ScreenerClient({
  ...
  useEffect(() => {
    ...
    try {
      stored = parseFavoriteSlugs(window.localStorage.getItem(FAVORITES_STORAGE_KEY));
    } catch {
      // 저장소 접근이 막힌 브라우저에서도 현재 세션의 별표는 계속 동작한다.
    }
    setFavoriteSlugs(stored);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 저장 실패는 필터와 현재 세션의 별표 상태를 막지 않는다.
    }
              ...
              <tr key={c.slug} className="group border-b border-[var(--color-border)]/50 hover:bg-[var(--color-panel-2)]">
                ...
                <td className={`${stickyBase} bg-[var(--color-panel)] group-hover:bg-[var(--color-panel-2)] px-3 py-2.5`}>
                  <div className="flex items-center gap-2">
                    ...
                    {newSlugs.has(c.slug) && (
                      <span className="shrink-0 rounded-full bg-blue-400/15 px-1.5 py-0.5 text-[10px] font-semibold text-blue-300" title="최근 30일 등록 또는 7일 수수료 50% 이상 성장·30일 현금흐름 $10K 이상">
                        신규 감지
                      </span>
                    )}
                  </div>
